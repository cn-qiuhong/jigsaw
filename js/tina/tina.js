...
var Tina = function (ops) {
	...
	T.setup = function (id, options) {
		...
		console.log("w: " + T.cvs.width + " h: " + T.cvs.height);
		...
		T.scale = options.scale || {x: _pixelRatio, y: _pixelRatio};

		T.canvas_tran_x = options.x || 0;
		T.canvas_tran_y = options.y || 0;
		T.width = parseInt(T.cvs.width - T.canvas_tran_x);
		T.height = parseInt(T.cvs.height - T.canvas_tran_y);
		// TODO: 处理屏幕旋转
	...
	T.load = function (assets, callback, ops) {
		...
		var progressCallback = ops.progressCallback || function (status, total) {
				...
				//T.ctx.restore();
				var x = T.width / 2 + T.canvas_tran_x;
				var y = T.height / 2 + T.canvas_tran_y;
				T.clear(T.options.clearColor);
				ctx.save();
				var startAngle = -Math.PI / 2;
				var value = status / total;
